    ...
    def get_saturday_exit_suggestions_for_period(self, matricula, start_date, end_date):
        ...
        while current <= end_dt:
            if current.weekday() == 5:
                ...
                available_minutes = max(0, float(bh_before or 0)) + max(0, int(extras_before or 0)) * MINUTOS_UNIDADE_EXTRA
                required_minutes = max(0, MINUTOS_JORNADA_SABADO - available_minutes)
                suggestion = {
                    ...
                    "minutos_compensaveis": available_minutes,
                    "minutos_a_trabalhar": required_minutes,
                    "saida_sugerida": "--:--",
                    "observacao": "Sem entrada registrada"
                }
                if entry_dt:
                    suggested_exit = entry_dt + timedelta(minutes=required_minutes)
                    suggestion["saida_sugerida"] = suggested_exit.strftime("%H:%M")
                    if required_minutes == 0:
                        suggestion["observacao"] = "Pode encerrar ao chegar"
                    else:
                        suggestion["observacao"] = "Saída calculada com base no saldo disponível ao iniciar o sábado"
