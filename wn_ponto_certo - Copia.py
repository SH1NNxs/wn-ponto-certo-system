...
class DatabaseManager:
    ...
    def get_point_panorama(self, start_date, end_date, target_matricula=None):
        ...
        for func_base in funcionarios:
            ...
            # --- INÍCIO DO CICLO WHILE REFEITO PARA SINCRONIZAÇÃO TOTAL ---
            while current_sim_iter <= sim_end_dt:
                ...
                # Se chegarmos no dia de hoje, apenas registramos o saldo acumulado até ontem
                ...
                # 1. Pega a expectativa de débito do dia (8h, 4h ou 0h)
                expected_minutes = self.get_expected_daily_minutes(data_str, is_fichado, matricula)

                # 2. BUSCA DINÂMICA DO TRABALHO REALIZADO:
                # Tentamos pegar do work_map (cache), mas validamos com a função de sumário real
                # Isso resolve o problema de o saldo não mudar após salvar alterações manuais.
                carga_real_str, _ = self._get_daily_summary_for_display(matricula, data_str)
                minutos_trabalhados_dia = parse_hhmm_to_minutes(carga_real_str)

                # 3. Cálculo do excedente/débito e atualização do saldo simulado
                excedente_dia = minutos_trabalhados_dia - expected_minutes
                saldo_bh_simulado += excedente_dia

                # 4. Normalização Cíclica (Conversão de Extras 4h/240min)
                ...
                # 5. Aplica Punições registradas no Banco de Dados
                punicao_do_dia = self.get_total_punishment_minutes_for_day(matricula, data_str)
                if punicao_do_dia > 0:
                    saldo_bh_simulado -= punicao_do_dia
                    # Re-normaliza após a punição
                    while saldo_bh_simulado < 0 and saldo_extras_simulado > 0:
                        saldo_bh_simulado += MINUTOS_UNIDADE_EXTRA
                        saldo_extras_simulado -= 1

                # 6. Registra no histórico para que a Treeview mostre os valores corretos linha a linha
                historico_simulado[data_str] = {
                    'bh_anterior': bh_anterior_simulado,
                    'bh_saldo': saldo_bh_simulado,
                    'extras_anterior': extras_anterior_simulado,
    ...
    def recalculate_full_balance_for_employee(self, matricula):
        ...
        # 1. SALDO INICIAL: Começa com os valores definidos no Marco Zero
        saldo_bh = func_info.get('banco_horas_inicial', 0)
        saldo_extras = func_info.get('extras_disponiveis_inicial', 0)

        # 2. BUSCA HISTÓRICO: Pega todos os dias trabalhados desde o início
        c.execute("""
            SELECT data, minutos_totais 
            FROM horas_trabalhadas 
            WHERE matricula=? AND data >= ? 
            ORDER BY data ASC
        """, (matricula, SYSTEM_START_DATE))
        ...
        try:
            ...
            # Recalcula até hoje para garantir que o saldo visual esteja em dia
            end_dt = date.today() 
        except: 
            return

        # 3. LOOP DIÁRIO: Processa dia a dia convertendo BH em Extras
        curr = start_dt
        while curr <= end_dt:
            d_str = curr.isoformat()
            trabalhado = work_map.get(d_str, 0)
            
            # Pega expectativa (480 min seg-sex / 240 min sab / 0 feriado/domingo)
            esperado = self.get_expected_daily_minutes(d_str, is_fichado, matricula)

            # Para não-fichados que não trabalharam no dia, a expectativa de débito é 0
            if not is_fichado and trabalhado == 0:
                esperado = 0

            # Adiciona o saldo do dia (trabalhado - esperado) ao Banco de Horas
            saldo_bh += (trabalhado - esperado)

            # NORMALIZAÇÃO: A cada 240 min (4h), gera 1 Extra e reduz o BH
            while saldo_bh >= 240:
                saldo_bh -= 240
                saldo_extras += 1
            
            # Se o saldo do dia for negativo, consome Extras para cobrir (se houver)
        ...
        # 4. PUNIÇÕES: Abate do Banco de Horas acumulado
        c.execute("SELECT SUM(minutos_descontados) as t FROM punicoes WHERE matricula=? AND data_punicao >= ?", (matricula, SYSTEM_START_DATE))
        total_punicoes = c.fetchone()['t'] or 0
        saldo_bh -= total_punicoes

        # 5. PAGAMENTOS: Deduz o que já foi pago manualmente (Extras e BH)
        ...
        for log in c.fetchall():
            try:
                ...
                if ant_extra and nov_extra:
                    saldo_extras -= (int(ant_extra.group(1)) - int(nov_extra.group(1)))

                # Deduz BH pago
                ant_bh = re.search(r'BH: ([\-\d:]+)', log['periodos_antigos'])
                nov_bh = re.search(r'BH: ([\-\d:]+)', log['periodos_novos'])
                if ant_bh and nov_bh:
                    saldo_bh -= (parse_hhmm_to_minutes(ant_bh.group(1)) - parse_hhmm_to_minutes(nov_bh.group(1)))
            except: 
                continue

        # 6. NORMALIZAÇÃO FINAL: Garante consistência antes de salvar
        while saldo_bh >= 240:
            saldo_bh -= 240
            saldo_extras += 1
        while saldo_bh < 0 and saldo_extras > 0:
        ...
        # 7. PERSISTÊNCIA: Grava e confirma a alteração no banco
        self.conn.execute("""
            UPDATE funcionarios 
            SET banco_horas=?, extras_disponiveis=? 
            WHERE matricula=?
        """, (saldo_bh, saldo_extras, matricula))
        
        self.conn.commit() # ESSENCIAL para o saldo mudar no sistema
...
class App:
    ...
    def load_point_viewer(self, force_reload=False):
        if self.unsaved_edits and not force_reload:
            if not messagebox.askyesno("Atualizar", "Alterações não salvas. Continuar?"): 
                return
            self.unsaved_edits = {}
        
        if hasattr(self, 'holiday_listbox'): 
            self.holiday_listbox.delete(0, tk.END)
        if hasattr(self, 'punishment_listbox'): 
            self.punishment_listbox.delete(0, tk.END)
        if hasattr(self, 'lbl_total_punishments'): 
        ...
        if not start_date or not end_date:
            ...
            for lbl in [getattr(self, 'lbl_total_punishments', None)]:
                if lbl: 
                    lbl.config(text="Total Punições: --")
            return

        # REMOVA ESTE BLOCO PROBLEMÁTICO (linhas 4195-4205 aproximadamente):
        # A lógica do horário de saída no sábado foi movida para a função on_export_panorama
        # NÃO execute este código aqui, apenas na exportação do PDF
        ...
        for i, item in enumerate(panorama_data):
            ...
            data_ptbr = datetime.strptime(data_db_str, "%Y-%m-%d").strftime("%d/%m/%Y") if data_db_str else data_db_str
            punicao_minutos = self.db.get_total_punishment_minutes_for_day(item['Matricula'], data_db_str)
            punicao_hms = format_minutes_to_hms(punicao_minutos) if punicao_minutos > 0 else "00:00:00"
        ...
        item['E2'], 
        item['S2'], 
        item['Carga_Horaria'], # Este valor deve vir do banco agora
        punicao_hms, 
        item['Total_Desconto']
    )
            ...
            self.tree_viewer.insert("", "end", values=values, iid=(item['Matricula'], item['Data']), 
                                tags=tuple(tags_para_linha))
        ...
        if target_matricula:
            ...
            fichado_val = func_info.get('fichado', 0)
            fichado_str = "Sim" if fichado_val == 1 else "Não"
            setor_str = func_info.get('setor', 'N/D')
            self.lbl_fichado_status.config(text=fichado_str)
            self.lbl_setor_status.config(text=setor_str)
        else:
            self.lbl_saldo_bh_total.config(text="--:--:--")
            self.lbl_saldo_extras_total.config(text="--")
            self.lbl_fichado_status.config(text="--")
        ...
        if start_date and end_date and hasattr(self, 'holiday_listbox'):
            ...
            [self.holiday_listbox.insert(tk.END, f"{datetime.strptime(h['data'], '%Y-%m-%d').strftime('%d/%m/%Y')} - {h['descricao']} ({h['tipo']})") 
            for h in holidays if h.get('data')]

        if target_matricula and start_date and end_date and hasattr(self, 'punishment_listbox'):
            punishments = self.db.get_punishments_in_range(target_matricula, start_date, end_date)
            total_punishments = len(punishments)
            self.lbl_total_punishments.config(text=f"Total Punições: {total_punishments}")
            [self.punishment_listbox.insert(tk.END, f"{datetime.strptime(p['data_punicao'], '%Y-%m-%d').strftime('%d/%m/%Y')} - {format_minutes_to_hms(p['minutos_descontados'])} - {p.get('motivo','Sem motivo')}") 
            for p in punishments if p.get('data_punicao')]
    ...
    def on_export_panorama(self):
        ...
        if not selected_func_str or selected_func_str == "Todos":
            messagebox.showerror("Erro", "Selecione um funcionário para gerar o espelho.")
            return
        ...
        # --- LÓGICA DE SALDO INICIAL DINÂMICO ---
        
        func_info_calc = self.db.get_funcionario_info(target_matricula)
        is_fichado_calc = func_info_calc.get('fichado', 0) == 1
        
        # 1. Determinar o Saldo de Início do Período
        # Se a data de início do relatório for IGUAL à data de início do sistema (Marco Zero)
        if start_date.isoformat() == SYSTEM_START_DATE:
            saldo_bh_entrada_relatorio = func_info_calc.get('banco_horas_inicial', 0)
            extras_entrada_relatorio = func_info_calc.get('extras_disponiveis_inicial', 0)
        else:
            # Caso contrário, buscamos o saldo que sobrou exatamente no dia anterior ao início deste relatório
            day_before = start_date - timedelta(days=1)
            try:
                # Simulamos o panorama apenas para o dia anterior para pegar o BH_Saldo real do banco
                day_before_data = self.db.get_point_panorama(day_before, day_before, target_matricula)
                if day_before_data:
                    saldo_bh_entrada_relatorio = parse_hhmm_to_minutes(day_before_data[0].get('BH_Saldo', '00:00:00'))
                    extras_entrada_relatorio = int(day_before_data[0].get('Extras_Disp', 0))
                else:
                    saldo_bh_entrada_relatorio = 0
                    extras_entrada_relatorio = 0
            except:
                saldo_bh_entrada_relatorio = 0
                extras_entrada_relatorio = 0
        ...
        saldo_bh_acumulado = saldo_bh_entrada_relatorio
        extras_acumuladas = extras_entrada_relatorio
        ...
        while current_date_iter <= end_date:
            ...
            # Acumula apenas até sexta para o fechamento do relatório
            if weekday < 5:
                saldo_bh_acumulado += (worked_today - expected_min)
                
                # Normalização de 4 horas
        ...
        # 2. Cálculo da Saída do Sábado (Baseado no saldo residual acumulado até sexta)
        saturday_date = None
        saturday_exit_info = "11:30 (Padrão)"
        
        for item in panorama_data:
            dt_obj = datetime.strptime(item['Data'], '%Y-%m-%d').date()
            if dt_obj.weekday() == 5: # Sábado
                saturday_date = dt_obj
                minutos_trabalho_sabado = 240 - saldo_bh_acumulado
                if minutos_trabalho_sabado < 0: minutos_trabalho_sabado = 0
                
                entrada_base = datetime.combine(dt_obj, time(7, 30))
                saida_calc = entrada_base + timedelta(minutes=minutos_trabalho_sabado)
                saturday_exit_info = saida_calc.strftime("%H:%M")
                break
        ...
        try:
            ...
            style_title = styles['h1']
            style_title.textColor = colors.teal
            style_table_header = ParagraphStyle(name='THead', parent=styles['Normal'], fontSize=8, fontName='Helvetica-Bold', alignment=TA_CENTER)
            style_table_body = ParagraphStyle(name='TBody', parent=styles['Normal'], fontSize=7, alignment=TA_CENTER)
            ...
            story.append(Paragraph(f"<b>Período:</b> {start_date.strftime('%d/%m/%Y')} a {end_date.strftime('%d/%m/%Y')}", styles['Normal']))
            story.append(Paragraph(f"<b>Saldo Início do Período (BH):</b> {format_minutes_to_hms(saldo_bh_entrada_relatorio)}", styles['Normal']))
            story.append(Spacer(1, 0.5*cm))

            # Tabela de registros
            col_headers = ["Dia", "Data", "E1", "S1", "E2", "S2", "Carga", "Punição", "Desconto"]
            table_data = [[Paragraph(h, style_table_header) for h in col_headers]]
            
            dias_semana = ["Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado", "Domingo"]
            for item in panorama_data:
                dt = datetime.strptime(item['Data'], '%Y-%m-%d')
                punicao = self.db.get_total_punishment_minutes_for_day(item['Matricula'], item['Data'])
                table_data.append([
                    dias_semana[dt.weekday()], dt.strftime('%d/%m/%Y'), item['E1'], item['S1'], 
                    item['E2'], item['S2'], item['Carga_Horaria'], format_minutes_to_hms(punicao), item['Total_Desconto']
                ])

            t = Table(table_data, colWidths=[2.5*cm, 2.5*cm, 2*cm, 2*cm, 2*cm, 2*cm, 2.5*cm, 2.5*cm, 2.5*cm])
            t.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.teal), ('TEXTCOLOR', (0,0), (-1,0), colors.whitesmoke), ('GRID', (0,0), (-1,-1), 1, colors.black), ('ALIGN', (0,0), (-1,-1), 'CENTER')]))
            story.append(t)
            story.append(Spacer(1, 0.5*cm))

            # Detalhamento Final
            detalhe_data = [
                ["Carga Horária Exigida:", format_minutes_to_hms(total_exigido_min)],
                ["Carga Horária Trabalhada:", format_minutes_to_hms(total_trabalhado_min)],
                ["Extras Geradas no Período:", f"{int(extras_acumuladas - extras_entrada_relatorio)} un."],
                ["Saldo Final (BH até Sexta):", format_minutes_to_hms(saldo_bh_acumulado)],
                [f"Saída no Sábado ({saturday_date.strftime('%d/%m') if saturday_date else ''}):", saturday_exit_info]
            ]
            
            t_det = Table(detalhe_data, colWidths=[8*cm, 4*cm])
            t_det.setStyle(TableStyle([('GRID', (0,0), (-1,-1), 0.5, colors.grey), ('BACKGROUND', (0,0), (0,-1), colors.lightgrey), ('FONTNAME', (0,0), (0,-1), 'Helvetica-Bold')]))
            story.append(t_det)
    ...
    def calculate_saturday_exit_time(self, matricula, saturday_date, remaining_bh_minutes):
        ...
        try:
            ...
            try:
                ...
                return "11:30"  # Fallback se formato inválido
    ...
    def commit_all_changes(self, from_exit=False):
        if self.editing_widgets: 
            [w.event_generate('<FocusOut>') for k, w in self.editing_widgets.items() if k == 'entry']
        ...
        self.append_log(f"Iniciando persistência de {len(self.unsaved_edits)} alterações...")
        ...
        self.append_log("Recalculando saldos totais...")
        for mat in affected_employees:
            try:
                # 1. Garante que o banco processe a nova lógica cíclica
                self.db.recalculate_full_balance_for_employee(mat)
            except Exception as e:
                self.append_log(f"ERRO CRÍTICO no recálculo de {mat}: {e}")

        self.unsaved_edits = {}

        # O SEGREDO: Forçar a interface a recarregar os dados do banco para a tela
        if not from_exit:
            self.load_point_viewer(force_reload=True) 
            messagebox.showinfo("Sucesso", "Alterações persistidas e saldos atualizados visualmente.")
