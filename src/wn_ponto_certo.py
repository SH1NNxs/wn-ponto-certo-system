...
def calculate_deduction(minutes_late):
    ...
    elif minutes_late <= 10: return minutes_late * 2
    elif minutes_late <= 15: return minutes_late * 3
    else: return minutes_late
...
class DatabaseManager:
    ...
    def get_point_panorama(self, start_date, end_date, target_matricula=None):
        ...
        sql_horas = f"SELECT matricula, data, periodos, minutos_totais FROM horas_trabalhadas WHERE matricula IN ({placeholders}) ORDER BY data ASC"
        c.execute(sql_horas, matriculas_list)
        ...
        for func in funcionarios:
            ...
            historico_simulado = {}
            for row in todos_os_dados_historicos:
                if row['matricula'] != matricula: continue
            ...
            while current_date <= end_dt:
                data_str = current_date.isoformat()
                dados_do_dia = next((r for r in todos_os_dados_historicos if r['matricula'] == matricula and r['data'] == data_str), None)
...
class App:
    ...
    def setup_styles(self):
        ...
        style.configure('TButton', font=('Segoe UI', 10, 'bold'), background=ACCENT_COLOR, foreground='white', borderwidth=0, focusthickness=0, padding=8)
        style.map('TButton', background=[('active', '#006666')])

        style.configure("Treeview", rowheight=25, fieldbackground=LIGHT_BG, background=LIGHT_BG, foreground=FG_COLOR)
        ...
        # << CORREÇÃO: Estilo global para todos os campos de entrada >>
        ...
        style.configure('TCombobox', fieldbackground=LIGHT_BG, background=LIGHT_BG, arrowcolor=FG_COLOR, foreground=FG_COLOR, selectbackground=LIGHT_BG, selectforeground=FG_COLOR)
        
        self.root.option_add('*TCombobox*Listbox.background', LIGHT_BG)
        ...
        style.configure('TLabelframe.Label', foreground=HIGHLIGHT_COLOR, background=BG_COLOR, font=('Segoe UI', 9, 'bold'))
        ...
        def save_changes():
            ...
            if cmb_extras_choice.get() == "Sim":
                try: 
                    ...
                    if unidades_a_deduzir > func_info.get('extras_disponiveis', 0):
                        messagebox.showerror("Erro de Validação", f"Não é possível deduzir {unidades_a_deduzir} unidade(s) de extra.\nSaldo disponível: {int(func_info.get('extras_disponiveis', 0))}", parent=win)
                        return
                except: messagebox.showerror("Erro", "Quantidade de extras deve ser um número inteiro.", parent=win); return
            ...
            if messagebox.askyesno("Confirmar", f"Deseja deduzir {unidades_a_deduzir} extra(s) e {format_minutes_to_hms(minutos_a_deduzir)} do saldo de {selection.split(' - ')[1]}?", parent=win):
    ...
    def setup_point_viewer(self, parent_frame):
        ...
        frame_viewer.grid_rowconfigure(2, weight=1)
        frame_viewer.grid_columnconfigure(0, weight=1)

        frame_controls = tk.Frame(frame_viewer, bg="#0a192f")
        frame_controls.grid(row=0, column=0, sticky="ew", pady=10, padx=10)
        ...
        tk.Label(frame_controls, text="De:", bg="#0a192f", fg="white").pack(side=tk.LEFT, padx=(0,5))
        self.cal_start_viewer = Calendar(frame_controls, selectmode="day", date_pattern="yyyy-mm-dd", locale='pt_BR', background="#008080", foreground="white", headersbackground="#008080")
        self.cal_start_viewer.pack(side=tk.LEFT, padx=(0, 10))
        
        tk.Label(frame_controls, text="Até:", bg="#0a192f", fg="white").pack(side=tk.LEFT, padx=(0,5))
        self.cal_end_viewer = Calendar(frame_controls, selectmode="day", date_pattern="yyyy-mm-dd", locale='pt_BR', background="#008080", foreground="white", headersbackground="#008080")
        self.cal_end_viewer.pack(side=tk.LEFT, padx=(0, 20))
        
        ttk.Button(frame_controls, text="Atualizar Tabela", command=self.load_point_viewer).pack(side=tk.LEFT, padx=5)
        ttk.Button(frame_controls, text="💾 Salvar Alterações", command=self.commit_all_changes).pack(side=tk.LEFT, padx=5)

        frame_saldos = ttk.LabelFrame(frame_viewer, text=" Saldos Atuais do Funcionário Selecionado ", style='TLabelframe')
        frame_saldos.grid(row=1, column=0, sticky="ew", pady=(10, 5), padx=10)
        ...
        self.cal_start_viewer.selection_set(today_str)
        self.cal_end_viewer.selection_set(today_str)
        self.load_point_viewer(force_reload=True)
    ...
    def load_point_viewer(self, force_reload=False):
        ...
        start_date, end_date = self.cal_start_viewer.get_date(), self.cal_end_viewer.get_date()
        ...
        for i, item in enumerate(panorama_data):
            data_ptbr = datetime.strptime(item['Data'], "%Y-%m-%d").strftime("%d/%m/%Y")
